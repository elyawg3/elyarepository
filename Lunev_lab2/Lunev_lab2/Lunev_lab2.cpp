...
void createPipe()
{
	...
	for (;;) {
		if (cin.fail()) {
			...
			if (Pipe[CountPipe].Length <= 0) {
				cout << "Длина трубы должна быть больше 0!\n";
				cout << "Введите длину трубы: ";
				cin >> Pipe[CountPipe].Length;
			}
		}
		if ((Pipe[CountPipe].Length > 0) && (!cin.fail())) {
			break;
		}
	}
	cout << "Введите диаметр трубы: ";
	cin >> Pipe[CountPipe].Diameter;
	for (;;) {
		if (cin.fail()) {
			cout << "Ошибка! Данные некорректны\n";
			CinClear();
			...
			if (Pipe[CountPipe].Diameter <= 0) {
				cout << "Диаметр трубы должен быть больше 0!\n";
				cout << "Введите диаметр трубы: ";
				cin >> Pipe[CountPipe].Diameter;
			}
		}
		if ((Pipe[CountPipe].Diameter > 0) && (!cin.fail())) {
			break;
		}
	}
	cout << "В ремонте(1 - да, 0 - нет): ";
	cin >> Pipe[CountPipe].InRepair;
	for (;;) {
		if (cin.fail()) {
			cout << "Ошибка! Данные некорректны\n";
			CinClear();
...
void ViewObject(const Station& cstation) {
	MenuClear();
	Pipe Pipe[1000];
	if ((Pipe[CountPipe].Length > 0) || (Pipe[CountPipe].Diameter > 0)) {
		cout << "Длина трубы: " << Pipe[CountPipe].Length << endl << "Диаметр трубы: " << Pipe[CountPipe].Diameter << endl << "В ремонте: "
			<< Pipe[CountPipe].InRepair << endl;
	}
	else {
